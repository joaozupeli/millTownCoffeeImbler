...
function buildMessage() {
  const specialtyLabel =
    specialties.find((item) => item.id === form.specialty)?.title || 'Not specified'

  return [
    `Hi Mill Town! My name is ${form.name || '(name)'}.`,
    `Phone: ${form.phone || '(not provided)'}.`,
    `Menu interest: ${specialtyLabel}.`,
...
function onSubmit() {
  ...
  const subject = encodeURIComponent('Mill Town Coffee — visit / menu question')
      ...
      <div class="contact__grid">
        <div class="contact__info">
          ...
          <div data-anim class="info">
            ...
            <p class="info__value">
              <a :href="clinic.instagramUrl || clinic.social.instagram" target="_blank" rel="noopener noreferrer">{{ clinic.phone }}</a><br />
        ...
        <form data-anim class="form" @submit.prevent="onSubmit">
          ...
          <label class="field">
            ...
            <input v-model="form.phone" type="tel" autocomplete="tel" placeholder="Optional" />
          </label>

          <label class="field">
            <span class="mono">Menu interest</span>
            <select v-model="form.specialty" required>
              <option value="" disabled>Pick a favorite</option>
          ...
          <label class="field">
            ...
            <textarea v-model="form.message" rows="3" placeholder="Hours, catering, or just saying hi" />
          </label>
